...
MAX_RUN_TIME = 16 * 24 * 60 * 60  # 16 dias em segundos
...
async def monitorar_tempo():
    while True:
        if time.time() - START_TIME > MAX_RUN_TIME:
            print("16 dias se passaram, desligando o bot...")
            await client.close()  # ou await bot.close() se usar commands.Bot
            break
        await asyncio.sleep(3600)  # espera 1h antes de checar de novo
...
async def on_ready():
    ...
    async def on_ready():
        ...
        bot.loop.create_task(monitorar_tempo())
