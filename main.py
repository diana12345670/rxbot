...
@app.route('/')
def home():
    return "🤖 RXbot está online!"

@app.route('/ping')
def ping():
    return {"status": "online", "bot": bot.user.name if bot.is_ready() else "starting"}
...
def start_keep_alive():
    ...
    flask_thread.start()
    logger.info("✅ Servidor keep-alive iniciado na porta 5000")

# Sistema de auto-restart melhorado
...
async def on_ready():
    ...
    if not hasattr(bot, '_tasks_started'):
        ...
        try:
            ...
            # Iniciar monitor de saúde do bot
            asyncio.create_task(monitor_bot())
            
            logger.info("✅ Background tasks e monitor iniciados")
        except Exception as e:
...
async def on_disconnect():
    ...
    try:
        ...
        channel = bot.get_channel(CHANNEL_ID_ALERTA)
        if channel:
            await channel.send("❌ O bot foi **desconectado** do Discord! Sistema de restart automático ativado...")
            
        # Ativar restart automático após desconexão
        asyncio.create_task(restart_bot())
        
    except Exception as e:
        logger.error(f"Erro ao lidar com desconexão: {e}")

@bot.event
async def on_resumed():
    logger.info("🔄 BOT RECONECTADO AO DISCORD!")
    global restart_count
    restart_count = 0  # Reset contador de restart em reconexão bem-sucedida
    ...
    try:
        ...
        if channel:
            embed = create_embed(
                ...
                f"**Keep-alive:** ✅ Servidor ativo na porta 5000",
                color=0x00ff00
            )
            await channel.send(embed=embed)
    except Exception as e:
...
# Evento para detectar problemas de conexão
@bot.event
async def on_socket_response(msg):
    """Monitor de socket para detectar problemas de conexão"""
    if msg.get('op') == 7:  # RECONNECT
        logger.warning("🔄 Discord solicitou reconexão")
        asyncio.create_task(restart_bot())
    elif msg.get('op') == 9:  # INVALID_SESSION
        logger.warning("🚨 Sessão inválida detectada")
        asyncio.create_task(restart_bot())
...
async def inventario(ctx, user: discord.Member = None):
    ...
    try:
        ...
        inventory_data = user_data[10] if len(user_data) > 10 else "{}"
        inventory = json.loads(inventory_data) if inventory_data else {}
...
async def restart_bot():
    """Sistema de restart automático do bot"""
    global restart_count
    ...
    restart_count += 1
    logger.info(f"🔄 Reiniciando bot... (Tentativa {restart_count}/{max_restarts})")
    ...
    try:
        ...
        try:
            ...
            if channel:
                embed = create_embed(
                    "🔄 Bot Reiniciando",
                    f"Sistema de restart automático ativado!\n"
                    f"**Tentativa:** {restart_count}/{max_restarts}\n"
                    f"**Hora:** <t:{int(datetime.datetime.now().timestamp())}:F>",
                    color=0xffaa00
                )
                await channel.send(embed=embed)
        except:
        ...
        restart_count = 0  # Reset contador em caso de sucesso
        return True
...
async def monitor_bot():
    """Monitor de saúde do bot com restart automático"""
    while True:
        try:
            await asyncio.sleep(300)  # Verificar a cada 5 minutos
            
            if bot.is_closed():
                logger.warning("🚨 Bot desconectado detectado!")
                await restart_bot()
            elif bot.latency > 30:  # Latência muito alta
                logger.warning(f"⚠️ Latência alta detectada: {bot.latency}s")
                await restart_bot()
...
async def start_bot():
    """Sistema de inicialização melhorado com restart automático"""
    reconnect_count = 0
    max_reconnects = 20  # Aumentado para mais tentativas
    ...
    while reconnect_count < max_reconnects:
        try:
            ...
            # Tentar restart automático após muitas tentativas
            if reconnect_count >= 5:
                logger.warning("🔄 Muitas tentativas falharam, tentando restart automático...")
                if await restart_bot():
                    reconnect_count = 0  # Reset contador se restart funcionou

        except discord.ConnectionClosed as e:
            logger.error(f"🔗 Conexão fechada: {e}")
            reconnect_count += 1
            
            # Tentar restart automático para connection closed
            if e.code in [1000, 1001, 1006]:  # Códigos que indicam desconexão limpa ou problema de rede
                logger.info("🔄 Tentando restart automático para conexão fechada...")
                if await restart_bot():
                    reconnect_count = 0
                    continue
            ...
            # Tentar restart automático para erros críticos
            if reconnect_count >= 3:
                logger.warning("🔄 Muitos erros, tentando restart automático...")
                if await restart_bot():
                    reconnect_count = 0
                    continue
    ...
    # Último recurso: restart automático total
    logger.error("💀 Erro crítico detectado! Tentativa de restart total...")
    
    try:
        # Tentar restart automático uma última vez
        if await restart_bot():
            logger.info("✅ Restart de emergência bem-sucedido!")
            # Reiniciar o loop de conexão
            await start_bot()
        else:
            # Se tudo falhar, tentar restart do processo
            import sys
            logger.error("💀 Restart de emergência falhou. Tentando restart do processo...")
            await bot.close()
            await asyncio.sleep(5)
        ...
        logger.error(f"❌ Falha crítica no restart: {e}")
        
        # Notificar erro crítico se possível
        try:
            channel = bot.get_channel(CHANNEL_ID_ALERTA)
            if channel:
                embed = create_embed(
                    "💀 ERRO CRÍTICO",
                    f"Sistema falhou completamente!\n"
                    f"**Erro:** {str(e)[:500]}\n"
                    f"**Hora:** <t:{int(datetime.datetime.now().timestamp())}:F>\n"
                    f"**Restart manual necessário!**",
                    color=0xff0000
                )
                await channel.send(embed=embed)
        except:
...
if __name__ == "__main__":
    try:
        ...
        logger.info("🚀 Iniciando RXbot com sistema keep-alive...")
        ...
        logger.info("🌐 Keep-alive ativo em: https://[SEU_REPL].repl.co")
        logger.info("📡 Configure UptimeRobot para: https://[SEU_REPL].repl.co/ping")

        # Iniciar bot com sistema de restart
        ...
        # Tentar restart automático mesmo em erro fatal
        try:
            logger.info("🔄 Tentando restart após erro fatal...")
            time.sleep(5)
            os.execl(sys.executable, sys.executable, *sys.argv)
        except:
            sys.exit(1)