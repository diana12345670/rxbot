...
async def close_http_session():
    ...
    if http_session and not http_session.closed:
        try:
            ...
            # Aguardar um pouco para garantir que tudo seja fechado
...
async def cleanup_resources():
    ...
    try:
        ...
        if collected > 0:
            logger.info(f"🧹 Limpeza automática: {collected} objetos removidos da memória")
        
        # Verificar se a sessão HTTP está funcionando
        global http_session
        if http_session and http_session.closed:
            logger.warning("⚠️ Sessão HTTP estava fechada - recriando...")
            http_session = None
    ...
    try:
        if not bot.is_closed():
            ...
            if channel:
                embed = create_embed(
                    "🚨 BOT EM ERRO CRÍTICO",
                    f"❌ Bot falhou após {max_reconnects} tentativas de reconexão!\n"
                    f"⚠️ Necessário intervenção manual ou restart do servidor.\n"
                    f"🔄 Tentativa de restart automático em andamento...",
                    color=0xff0000
                )
                await channel.send(embed=embed)
    except:
...
if __name__ == "__main__":
    ...
    while restart_count < max_restarts:
        try:
            ...
            logger.info("🔄 Task cancelada - tentando restart...")
            restart_count += 1
